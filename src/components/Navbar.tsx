...
import { LogOut, Menu, X, User, Plus, Shield, Sun, Moon } from 'lucide-react';
    ...
    <>
      ...
      >
        ...
        <div className="relative flex items-center justify-between px-6 md:px-12 py-5 max-w-content mx-auto">
            ...
            {!loading && (
              user ? (
                <div className="flex items-center gap-1 ml-2 pl-3 border-l border-white/[0.08]">
                  ...
                  <button
                    onClick={toggleTheme}
                    className="flex items-center text-white/40 px-2 py-2 rounded-full transition-all duration-300 hover:text-accent hover:bg-white/[0.04]"
                ...
                <div className="flex items-center gap-1 ml-3">
                  ...
                  <Link
                    to="/login"
                    className="font-inter text-[15px] font-medium text-black bg-accent px-6 py-2.5 rounded-full transition-all duration-300 hover:shadow-lg hover:shadow-accent/20 active:scale-[0.97]"
                  >
                    Login
                  </Link>
                </div>
          ...
          <div className="md:hidden pointer-events-auto flex items-center gap-1">
            <button
              onClick={toggleTheme}
              className="w-11 h-11 flex items-center justify-center rounded-full hover:bg-white/[0.05] transition-colors duration-300 text-white/60"
            >
              {theme === 'dark' ? <Sun className="w-4 h-4" /> : <Moon className="w-4 h-4" />}
            </button>
            <button
              onClick={() => setMobileOpen(!mobileOpen)}
              className="w-11 h-11 flex items-center justify-center rounded-full hover:bg-white/[0.05] transition-colors duration-300 text-white"
            >
